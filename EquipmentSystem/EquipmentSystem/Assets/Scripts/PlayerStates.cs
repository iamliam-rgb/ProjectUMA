...
public class PlayerStates : MonoBehaviour
{
    ...
    public CurrentGender currentGender;
    [Header("Gender")]
    [SerializeField] public List<GameObject> listOfAllGenderPrefabs; //list of all genders
    ...
    private void MonitorDesiredCharacterGender()
    {
        switch (currentGender)
        {
            ...
            case CurrentGender.female:
                for(int i = 0; i < listOfAllGenderPrefabs.Count; i++)
                {
                    if (i == 0)
                        listOfAllGenderPrefabs[i].SetActive(true);
                    else
                        listOfAllGenderPrefabs[i].SetActive(false);
                }
                break;
